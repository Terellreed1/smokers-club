...
const Shop = () => {
  ...
  const FilterGroup = ({ label, options, value, onChange }: { label: string; options: string[]; value: string; onChange: (v: string) => void }) => (
    <div className="mb-8">
      <h4 className="text-xs font-sans uppercase editorial-spacing text-gold mb-4">{label}</h4>
      ...
      <div className="py-16 md:py-24 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-xs font-sans uppercase editorial-spacing text-gold mb-4">Curated Selection</p>
            ...
            <aside>
              ...
              <div className="mb-8">
                <button
                  ...
                  className={`text-xs font-sans px-4 py-2 border transition-all duration-300 ${
                    dealsOnly ? "border-gold text-gold bg-gold/5" : "border-border/50 text-muted-foreground hover:border-gold/30"
                  }`}
                >
                  Deals Only
                </button>
                ...
                <Link key={product.id} to={`/shop/${product.id}`} className="group block">
                  <div className="aspect-[3/4] bg-secondary/80 border border-border/50 mb-4 overflow-hidden relative transition-all duration-500 group-hover:border-gold/30">
                    ...
                    <div className="absolute top-4 left-4 flex gap-2">
                      <span className="text-[10px] font-sans uppercase editorial-spacing text-gold bg-background/90 px-3 py-1">{product.type}</span>
                      {product.deal && (
                        <span className="text-[10px] font-sans uppercase editorial-spacing text-background bg-gold px-3 py-1">Deal</span>
                      )}
                    </div>
                  </div>
                  <p className="text-xs font-sans uppercase editorial-spacing text-muted-foreground mb-1">{product.brand}</p>
                  <h3 className="font-serif text-lg text-foreground group-hover:text-gold transition-colors duration-300">{product.name}</h3>
